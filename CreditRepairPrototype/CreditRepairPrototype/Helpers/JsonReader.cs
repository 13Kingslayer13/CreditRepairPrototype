...
namespace CreditRepairPrototype
{
	public class JsonReader : Singleton<JsonReader>
	{
		...
		public T ReadJson<T>(Assembly assembly, string resourceName)
		{
			string loadedData = GetEmbeddedResourceString (assembly, resourceName);
			T dataObj = JsonConvert.DeserializeObject<T>(loadedData);
			return dataObj;
		}

		public List<T> ReadJsonArrayToList<T>(Assembly assembly, string resourceName) where T : class, new()
		{
			string loadedData = GetEmbeddedResourceString (assembly, resourceName);
			List<T> dataList = JsonConvert.DeserializeObject<List<T>>(loadedData, new CustomConverter<T>());
			return dataList;
		}
	}

	public class CustomConverter<T> : CustomCreationConverter<T> where T : class, new()
    {
